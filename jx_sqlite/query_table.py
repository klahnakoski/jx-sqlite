...
from jx_base.expressions import TupleOp, Variable, jx_expression, QueryOp
    ...
    def query(self, query=None):
        ...
        if query.format == "container":
            ...
            if len(query.edges) == 0 and len(query.groupby) == 0:
                ...
                return Data(data=unwrap(data), select=select, meta={"format": "cube"})
            ...
            if (
                ...
                and any(listwrap(query.select).aggregate)
            ):
                if is_list(query.select):
                    data = Data()
                    for c in index_to_columns.values():

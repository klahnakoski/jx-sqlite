...
# ONE EDGE COMPILED TO SQL INGREDIENTS (P6a, docs/INTERSECTION_SURVEY.md).
# domains_sql:      THE DOMAIN TABLE SQL (THE COORDINATE SPACE FOR THIS EDGE)
# on_clause:        HOW A FACT ROW JOINS TO A DOMAIN COORDINATE
# join_type:        INNER (allowNulls=False) OR LEFT (allowNulls=True)
# domain_aliases:   THE DOMAIN COLUMN ALIASES
# column_mappings:  {absolute column index -> ColumnMapping} FOR THE EDGE COLUMNS
# next_column_index: THE RUNNING COLUMN COUNTER, ADVANCED PAST THIS EDGE
SqlDomain = namedtuple(
    "SqlDomain",
    ["domains_sql", "on_clause", "join_type", "domain_aliases", "column_mappings", "next_column_index"],
)
...
def sql_domain(query, query_edge, edge_index, column_index, inner_schema, from_sql):
    ...
    :param column_index: RUNNING COLUMN COUNTER (== len(index_to_column) AT ENTRY)
    :param inner_schema: SCHEMA TO COMPILE EDGE VALUES AGAINST
